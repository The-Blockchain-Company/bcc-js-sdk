...
describe('Transaction', () => {
  it('throws if inputs are invalid', () => {
    ...
    expect(() => Transaction(RustBcc, emptyInputArray, outputs)).to.throw(EmptyArray)
    expect(() => Transaction(RustBcc, invalidInputType, outputs)).to.throw(/Invalid value/)
  })
  ...
  it('throws if outputs are invalid', () => {
    ...
    expect(() => Transaction(RustBcc, inputs, emptyOutputArray)).to.throw(EmptyArray)
    expect(() => Transaction(RustBcc, inputs, invalidOutputType)).to.throw(/Invalid value/)
  })
  ...
  it('throws if a transaction has more combined output value than input value', () => {
    ...
    expect(() => Transaction(RustBcc, inputs, outputs)).to.throw(InsufficientTransactionInput)
  })
  ...
  it('accepts more combined input value than output, to cover fees', () => {
    ...
    expect(() => Transaction(RustBcc, inputs, outputs)).to.not.throw()
  })
  ...
  it('allows access to a transaction as hex', () => {
    ...
    const fee = Transaction(RustBcc, inputs, outputs).estimateFee()

    outputs[0].value = (6000000 - Number(fee)).toString()
    const transaction = Transaction(RustBcc, inputs, outputs)
  ...
  describe('fee', () => {
    it('returns the realised fee of a transaction', () => {
      ...
      const estimatedFee = Transaction(RustBcc, inputs, outputs).estimateFee()
      const realisedFee = Transaction(RustBcc, inputs, outputs).fee()

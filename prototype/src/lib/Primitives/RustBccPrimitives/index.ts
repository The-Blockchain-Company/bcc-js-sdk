import { Bcc, FeeAlgorithm, ChainSettings, TransactionSelection } from '../../../Bcc'
import { getBindingsForEnvironment } from '../../bindings'
import { InsufficientTransactionInput } from '../../../Transaction/errors'
import { TxInput as BccTxInput, Coin as CoinT, Bip44AccountPrivate } from 'bcc-wallet'
...
export function convertCoinToEntropic (coin: CoinT): string {
  const bcc = coin.bcc()
  const entropic = coin.entropic()
  return String((bcc * 1000000) + entropic)
}

export const RustBcc: Bcc = {
  buildTransaction: (inputs, outputs, feeAlgorithm = FeeAlgorithm.default) => {
    ...
    */
    ...
    const fee = convertCoinToEntropic(feeAsCoinType)
    ...
    */
    ...
    const feeEstimateAsEntropic = convertCoinToEntropic(feeEstimate)

    const bccTransaction = transactionBuilder.make_transaction()

    // This gets around WASM incorrectly freeing the tx allocation when
    // we want reference to a finalizer
    const txClone = Transaction.from_json(bccTransaction.to_json())
    let finalizer = new TransactionFinalized(bccTransaction)
    ...
    return {
      ...
      fee: () => fee,
      estimateFee: () => feeEstimateAsEntropic
    }
  },
  ...
  inputSelection (outputs: TransactionOutput[], utxoSet: UtxoWithAddressing[], changeAddress: string, feeAlgorithm = FeeAlgorithm.default): TransactionSelection {
    const potentialInputs: BccTxInput[] = utxoSet.map(utxo => {
      return TxInput.new(
    ...
    const estimatedChange = convertCoinToEntropic(selectionResult.estimated_change())

...
describe('RustBcc', () => {
  describe('verifyMessage', () => {
    it('returns true when verifying a correct signature for a message', async () => {
      ...
      const keyManager = InMemoryKeyManager(RustBcc, { mnemonic, password: 'securepassword' })
      const { signature, publicKey } = await keyManager.signMessage(AddressType.external, 0, message)

      const verification = RustBcc.verifyMessage({
        publicKey,
        message,
        signature
      })
  ...
  describe('inputSelection', () => {
    it('throws if there is insufficient inputs to cover the payment cost', async () => {
      ...
      const account = await InMemoryKeyManager(RustBcc, { password: '', mnemonic }).publicParentKey()
      const [address1, address2, address3, changeAddress] = addressDiscoveryWithinBounds(RustBcc, {
        account,
      ...
      expect(() => RustBcc.inputSelection(outputs, utxosWithAddressing, changeAddress.address)).to.throw('NotEnoughInput')
    })
    ...
    describe('FirstMatchFirst', () => {
      it('selects valid UTXOs and produces change', async () => {
        const mnemonic = generateMnemonic()
        const account = await InMemoryKeyManager(RustBcc, { password: '', mnemonic }).publicParentKey()
        const [address1, address2, address3, address4, address5, change] = addressDiscoveryWithinBounds(RustBcc, {
          account,
          type: AddressType.internal,
          lowerBound: 0,
          upperBound: 5
        ...
        const { inputs, changeOutput } = RustBcc.inputSelection(outputs, utxosWithAddressing, change.address)

...
describe('Wallet', () => {
  ...
  describe('getNextChangeAddress', () => {
    beforeEach(async () => {
      ...
      publicParentKey = await InMemoryKeyManager(RustBcc, { password: '', mnemonic }).publicParentKey()

      seedTransactionSet([])

      wallet = Wallet(RustBcc, mockProvider)({ publicParentKey })
    })
  ...
  describe('getNextReceivingAddress', async () => {
    ...
    it('returns the next receiving address for a new BIP44 account', async () => {
      const firstExternalAddress = addressDiscoveryWithinBounds(RustBcc, {
        account: publicParentKey,

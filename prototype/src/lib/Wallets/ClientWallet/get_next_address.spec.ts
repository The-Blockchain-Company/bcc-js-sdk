...
import { InMemoryKeyManager, RustBcc } from '../..'
...
describe('getNextAddressByType', () => {
  it('returns the first address index if no transactions exist for internal addresses', async () => {
    ...
    const firstInternalAddress = addressDiscoveryWithinBounds(RustBcc, {
      account,
    ...
    const nextChangeAddress = await getNextAddressByType(RustBcc, mockProvider, account, AddressType.internal)
  ...
  it('returns the first address index if no transactions exist for external addresses', async () => {
    ...
    const nextChangeAddress = await getNextAddressByType(RustBcc, mockProvider, account, AddressType.external)
    expect(nextChangeAddress.address).to.eql(firstInternalAddress)
    expect(nextChangeAddress.index).to.eql(0)

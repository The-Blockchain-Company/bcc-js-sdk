...
function convertUnitToEntropic (unit: RemoteUnit, value: number): number {
  return unit === RemoteUnit.entropic ? value : value * 1000000
}
...
export function RemoteWallet (walletProvider: WalletProvider, walletId: string): WalletInstance {
  return {
    getNextReceivingAddress: async () => {
      ...
      // Issue: https://github.com/The-Blockchain-Company/bcc-js-sdk/issues/34
    ...
    balance: async () => {
      ...
      return convertUnitToEntropic(remoteWallet.balance.available.unit, availableBalance)
    },
    transactions: async () => {
  ...
  return {
    ...
    inputs: remoteTransaction.inputs.map((input, index) => {
      return {
        ...
        value: {
          ...
          value: String(convertUnitToEntropic(input.amount.unit, input.amount.quantity))
        }
      }
    }),
    outputs: remoteTransaction.outputs.map(output => {
      return {
        ...
        value: String(convertUnitToEntropic(output.amount.unit, output.amount.quantity))
      }
    }),

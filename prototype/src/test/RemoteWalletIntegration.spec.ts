...
import BccSDK, { BccWalletProvider } from '..'
...
describe('Example: Interacting with remote wallets', function () {
  this.timeout(20000)
  // The node, wallet and seed are pulled from `Sam-Jeston/bcc-sl-cole`
  const localApiEndpoint = 'http://localhost:8088'

  let bcc: ReturnType<typeof BccSDK>
  let connection: ReturnType<typeof bcc.connect>
  const GENERIC_PASSPHRASE = 'Secure Passphrase'
  beforeEach(() => {
    bcc = BccSDK()
    connection = bcc.connect(BccWalletProvider(localApiEndpoint))
  })
    ...
    describe('Wallet', () => {
      ...
      describe('createAndSignTransaction', () => {
        it('fails to create a transaction for a wallet without funds', async () => {
          ...
          const newParentKey = await bcc.InMemoryKeyManager({ mnemonic, password: 'pw' }).publicParentKey()
          const { address } = await bcc.connect(mockProvider).wallet({ publicParentKey: newParentKey }).getNextReceivingAddress()

          const payment: RemotePayment = {
            address,
            amount: {
              quantity: 1000000,
              unit: RemoteUnit.entropic
            }
          }
        ...
        // Issue to follow up: https://github.com/The-Blockchain-Company/bcc-js-sdk/issues/33
        it.skip('creates a transaction for a funded wallet to an external address', async () => {
          ...
          const mnemonic = generateMnemonic()
          const newParentKey = await bcc.InMemoryKeyManager({ mnemonic, password: 'pw' }).publicParentKey()
          const { address } = await bcc.connect(mockProvider).wallet({ publicParentKey: newParentKey, chainSettings: ChainSettings.testnet }).getNextReceivingAddress()
      ...
      // INFO: Awaiting list transaction implementation in bcc-wallet

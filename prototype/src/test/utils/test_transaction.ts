...
export function generateTestTransaction ({
  ...
  const receiptAddresses = addressDiscoveryWithinBounds(RustBcc, {
    account,
  ...
  const changeAddresses = addressDiscoveryWithinBounds(RustBcc, {
    account,
  ...
  const fee = Transaction(RustBcc, inputs, testOutputs).estimateFee()

  testOutputs[0].value = (Number(testOutputs[0].value) - Number(fee)).toString()

  return { transaction: Transaction(RustBcc, inputs, testOutputs), inputs }
}

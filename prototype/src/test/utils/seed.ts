...
export async function generateSeed () {
  ...
  const account1 = await InMemoryKeyManager(RustBcc, { password: '', mnemonic: mnemonic1 }).publicParentKey()
  const account2 = await InMemoryKeyManager(RustBcc, { password: '', mnemonic: mnemonic2 }).publicParentKey()

  const account2Addresses = addressDiscoveryWithinBounds(RustBcc, {
    account: account2,
    lowerBound: 0,
    upperBound: 39,

import { CSL, Bcc, coreToCsl } from '@bcc-sdk/core';
...
const redistributeLeftoverAssets = (
  ...
  requestedAssetChangeBundles: Bcc.Value[],
  uniqueOutputAssetIDs: string[]
) => {
...
const coalesceChangeBundlesForMinCoinRequirement = (
  ...
  const satisfiesMinCoinRequirement = (valueQuantities: Bcc.Value) =>
  ...
  while (sortedBundles.length > 1 && !satisfiesMinCoinRequirement(sortedBundles[sortedBundles.length - 1])) {
    ...
    sortedBundles[sortedBundles.length - 1] = Bcc.util.coalesceValueQuantities([
...
const computeChangeBundles = ({
  ...
  computeMinimumCoinQuantity: ComputeMinimumCoinQuantity;
  fee?: bigint;
}): UtxoSelection & { changeBundles: Bcc.Value[] } => {
...
/**
 * 1. Compute change bundles with fee included and coalesce them to cover min BCC requirement.

import { AssetId, SelectionConstraints } from '@bcc-sdk/util-dev';
import { SelectionResult } from '../../src/types';
import { cslUtil, Bcc, CSL, cslToCore } from '@bcc-sdk/core';
...
const totalOutputsValue = (outputs: Set<CSL.TransactionOutput>) =>
  Bcc.util.coalesceValueQuantities([...outputs].map((output) => cslToCore.value(output.amount())));

const totalUtxosValue = (results: SelectionResult) =>
  Bcc.util.coalesceValueQuantities(
...
const inputSelectionTotals = ({
  ...
  const vChange = Bcc.util.coalesceValueQuantities(
...
export const assertFailureProperties = ({
  ...
  utxoAmounts: Bcc.Value[];
  outputsAmounts: Bcc.Value[];
  constraints: SelectionConstraints.MockSelectionConstraints;
  implicitCoin?: Bcc.ImplicitCoin;
}) => {
  const availableQuantities = Bcc.util.coalesceValueQuantities([
    ...utxoAmounts,
    { coins: BigInt(implicitCoin?.input || 0) }
  ]);
  const requestedQuantities = Bcc.util.coalesceValueQuantities([
    ...outputsAmounts,
...
export const generateSelectionParams = (() => {
  ...
  const arrayOfCoinAndAssets = (implicitCoin = 0n) =>
    fc
      ...
      .filter((values) => {
        ...
        const { coins, assets } = Bcc.util.coalesceValueQuantities(values);
        return (
  ...
  const generateImplicitCoin: Arbitrary<Bcc.ImplicitCoin | undefined> = fc.oneof(
    fc.constant(void 0),
    fc.record({
      deposit: fc.oneof(
        fc.constant(void 0),

...
import { WindowMaybeWithBcc } from '../injectWindow';
...
/**
 * Unique identifier, used to inject into the bcc namespace
 */
...
export class Wallet {
  ...
  public getPublicApi(window: WindowMaybeWithBcc) {
    return {
      name: this.name,
   ...
   * Errors: `ApiError`
   */
  public async isEnabled(window: WindowMaybeWithBcc): Promise<Boolean> {
   ...
   * Errors: `ApiError`
   */
  public async enable(window: WindowMaybeWithBcc): Promise<WalletApi> {
    const appName = window.location.hostname;

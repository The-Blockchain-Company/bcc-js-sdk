...
import { WalletProvider, ProviderError, ProviderFailure, Bcc } from '@bcc-sdk/core';
...
export const blockfrostProvider = (options: Options, logger = dummyLogger): WalletProvider => {
  ...
  const fetchRedeemers = async ({
    redeemer_count,
    hash
  }: Responses['tx_content']): Promise<Bcc.Redeemer[] | undefined> => {
    if (!redeemer_count) return;
    const response = await blockfrost.txsRedeemers(hash);
    return response.map(
      ({ purpose, script_hash, unit_mem, unit_steps, tx_index }): Bcc.Redeemer => ({
        index: tx_index,
        executionUnits: {
          memory: Number.parseInt(unit_mem),
          steps: Number.parseInt(unit_steps)
        },
        purpose: ((): Bcc.Redeemer['purpose'] => {
          switch (purpose) {
            case 'cert':
              return 'certificate';
            case 'reward':
  ...
  const fetchWithdrawals = async ({
    withdrawal_count,
    hash
  }: Responses['tx_content']): Promise<Bcc.Withdrawal[] | undefined> => {
    if (!withdrawal_count) return;
    const response = await blockfrost.txsWithdrawals(hash);
    return response.map(
      ({ address, amount }): Bcc.Withdrawal => ({
        address,
  ...
  const fetchMint = async ({
    ...
    hash
  }: Responses['tx_content']): Promise<Bcc.TokenMap | undefined> => {
  ...
  const fetchPoolRetireCerts = async (hash: string): Promise<Bcc.PoolCertificate[]> => {
    ...
    return response.map(({ cert_index, pool_id, retiring_epoch }) => ({
      ...
      poolId: pool_id,
      type: Bcc.CertificateType.PoolRetirement
    }));
  };

  const fetchPoolUpdateCerts = async (hash: string): Promise<Bcc.PoolCertificate[]> => {
    const response = await blockfrost.txsPoolUpdates(hash);
    return response.map(({ cert_index, pool_id, active_epoch }) => ({
      epoch: active_epoch,
      certIndex: cert_index,
      poolId: pool_id,
      type: Bcc.CertificateType.PoolRegistration
    }));
  };

  const fetchMirCerts = async (hash: string): Promise<Bcc.MirCertificate[]> => {
    const response = await blockfrost.txsMirs(hash);
    return response.map(({ address, amount, cert_index, pot }) => ({
      type: Bcc.CertificateType.MIR,
      address,
      quantity: BigInt(amount),
      certIndex: cert_index,
      pot
  ...
  const fetchStakeCerts = async (hash: string): Promise<Bcc.StakeAddressCertificate[]> => {
    const response = await blockfrost.txsStakes(hash);
    return response.map(({ address, cert_index, registration }) => ({
      type: registration ? Bcc.CertificateType.StakeRegistration : Bcc.CertificateType.StakeDeregistration,
      address,
      certIndex: cert_index
    }));
  };

  const fetchDelegationCerts = async (hash: string): Promise<Bcc.StakeDelegationCertificate[]> => {
    const response = await blockfrost.txsDelegations(hash);
    return response.map(({ cert_index, index, address, active_epoch, pool_id }) => ({
      type: Bcc.CertificateType.StakeDelegation,
      certIndex: cert_index,
  ...
  const fetchCertificates = async ({
    ...
    delegation_count,
    hash
  }: Responses['tx_content']): Promise<Bcc.Certificate[] | undefined> => {
  ...
  const fetchTransaction = async (hash: string): Promise<Bcc.TxAurum> => {
